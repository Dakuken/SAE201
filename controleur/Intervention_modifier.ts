...
if (modifierInfo === undefined) {
    class ModifierInfo {
        ...
        ChangerDetail(Parent : HTMLElement, id : string) {
            // ? Valeur de l'input
            const childvalue = (<HTMLInputElement>(Parent.firstChild)).value;

            // Message d'erreur peut apparaitre
            let erreur = (<HTMLParagraphElement>(document.querySelector(`[id=${id}]`).children[1]));
            if (Parent.id === 'heure_interv') {
                if (childvalue === '') { (<HTMLParagraphElement>document.querySelector('[id=erreurHeure]')).classList.remove('hide'); return; }
            ...
            if (erreur.id === 'erreurObjet') {
                if (childvalue === '') { this.messagErreur(erreur, childvalue); return; }
            ...
            let parentInListe: number;
            for (let i = 0; i <= tabId.length - 1; i ++) {
                if (id === tabId[i]) { parentInListe = i + 1; }
            }
            ...
            if (maliste.length !== 0) {
                ...
                for (let i = 0; i <= values.length - 1; i ++) {
                    const str = values[i];
                    const child = <HTMLParagraphElement>(<HTMLElement>document.querySelector(`[id=${tabId[i + parentInListe]}]`));
                    child.innerText = str;
                }
            } else { this.messagErreur(erreur, childvalue); }
        }

        messagErreur(selector:HTMLElement, childvalue:string) {
            const { id } = selector;
            if (childvalue !== '') {
                switch (id) {
                case 'Erreur_Cont': selector.innerText = 'Attention, aucun numéro de contrat ne correspond.'; selector.classList.remove('hide'); break;
                case 'Erreur_Cons': selector.innerText = 'Attention, aucun numéro de conseiller ne correspond'; selector.classList.remove('hide'); break;
                default: break;
                }
            } else {
                switch (id) {
                case 'erreurObjet': selector.innerText = "Attention, un motif d'intervention ne peut être vide"; selector.classList.remove('hide'); break;
                case 'Erreur_Cont': selector.innerText = 'Le numéro de contrat doit être renseigné.'; selector.classList.remove('hide'); break;
                case 'Erreur_Cons': selector.innerText = 'Le numéro de conseiller doit être renseigné.'; selector.classList.remove('hide'); break;
                default: break;
                }
            }
        }

        newInfoListe(id:string, childvalue:string):TtabAsso[] {
            switch (id) {
        ...
        validerModif() {
            ...
            for (let i = 0; i <= tabParentInputModif.length - 1; i ++) {
                ...
                if (tabParentInputModif[i].id === 'num_cont' || tabParentInputModif[i].id === 'num_cons') {
                    ...
                    const numCont = (<HTMLInputElement>(<HTMLDivElement>document.querySelector('[id=num_cont]').firstChild)).value;
                    const childValue = this.valueOfChild(tabParentInputModif[i]);
                    ...
                    let content = (<HTMLTextAreaElement>tabParentInputModif[i].querySelector('textarea')).value;
            ...
            (<HTMLDivElement>document.querySelector('[id=Div_Bouton_Modif]')).classList.add('hide');
        }

        valueOfChild(Parent:HTMLElement):string {
            return (<HTMLInputElement>(Parent.firstChild)).value;
        }

        verif(parent:HTMLElement, child:string):TtabAsso[] {
...
let TabBoutonAffichModif:HTMLElement[] = [<HTMLDivElement>document.querySelector('[id=Btn_RetourAffich]'), <HTMLDivElement>document.querySelector('[id=Div_Bouton_Prest]'), <HTMLDivElement>document.querySelector('[id=Btn_ValiderModif]'), <HTMLDivElement>document.querySelector('[id=Btn_AnnulerModif]')];
...
let tabParentInputModif:HTMLDivElement[] = [];
let tabSelectorError:HTMLDivElement[] = [<HTMLDivElement>document.querySelector('[id=Erreur_Cont]'),
<HTMLDivElement>document.querySelector('[id=Erreur_Cons]'),
<HTMLDivElement>document.querySelector('[id=Erreur_Date]')];

//* ----------------------------------Modifier----------------------------------/
// ?------- Modifier détails Interventions ---------/
(<HTMLInputElement>document.querySelector('[id=Btn_Modifier]')).addEventListener('click', () => {
