...
//* ---------------------------------AFFICHER-----------------------------------/
//* ---------------------------------------------------------------------------*/
export default class AfficherInfo {
    //* ------- INIT ---------/
    // on init la phrase erreur en hidden
    init(): void {
        let noSelect = (<HTMLParagraphElement>document.querySelector('[id=NoSelect]'));
        if (noSelect === undefined || noSelect === null) {
            throw new Error("Le selector n'existe pas ");
        }
    ...
    rowSelector(): void {
        ...
        if (rowSelected === undefined || rowSelected === null) {
            throw new Error('La ligne selectionnée n\'est pas définit ');
        }
    }
    ...
    Afficher_Modifer(htmlElement: string = 'p') {
        (<HTMLParagraphElement>(document.querySelector('[id=NoSelect]'))).style.visibility = 'hidden';
    ...
    hideAffichBtn() {
        (<HTMLDivElement>(document.querySelector('[id=Div_Retour_Affich]'))).classList.add('hide');
        (<HTMLDivElement>(document.querySelector('[id=Div_Bouton_Prest]'))).classList.remove('hide');
        (<HTMLDivElement>(document.querySelector('[id=Div_Bouton_Modif]'))).classList.remove('hide');
    }

    //* ------- Montrer bouton retour  ---------/
    showAffichBtn() {
        (<HTMLDivElement>(document.querySelector('[id=Div_Retour_Affich]'))).classList.remove('hide');
        (<HTMLDivElement>(document.querySelector('[id=Div_Bouton_Prest]'))).classList.add('hide');
