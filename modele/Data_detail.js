...
if (dataDetail === undefined) {
    class DataDetail {
        ...
        TableauValeur(htmlElement) {
            let tabresult;
            if (htmlElement === 'p') {
                tabresult = this.TabInfo();
            }
            else {
                tabresult = this.TabInfo(htmlElement);
            }
        ...
        TabInfo(element = 'p') {
            ...
            const cond = (element === 'input') ? ` AND intervention.num_interv = '${numInterv}' ` : '';
            gestionTable.remplirTable(cond);
            gestionTable.selectInterv(document.querySelector(`[id=${rowSelected.id}]`));
            return maliste;
        }
    }
    var dataDetail = new DataDetail();
}
//# sourceMappingURL=Data_detail.js.map